...
export default function CompletionBlock({ id = "NYI", prompts, onChange, idx }) {
    ...
    return (
        <div className="w-full" onChange={() => onChange(idx, {
            ...
            topP: document.getElementById(`top_pRange-${id}`).value
        })}>
            ...
            <div className="flex space-x-4">
                <div className="flex-grow box-border border-4 bg-gray-100 rounded-xl p-8">
                    ...
                    <div className="inline-block px-3">
                        <label htmlFor="topP" className="block">Top P:</label>
                        <input type="range" name="topP" min="1" max="100" defaultValue="50" id={`top_pRange-${id}`} />
                    </div>
                </div>
            </div>
        </div>
